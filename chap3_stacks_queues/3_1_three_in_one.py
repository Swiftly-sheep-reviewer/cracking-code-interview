...
class MultiStack:
    ...
    def __init__(self, stack_size, num_of_stacks):
        ...
        self.array = [0] * (stack_size * self.num_of_stacks)
...
def test_multistack():
    num_stacks = 3
    stack_size = 6
    stack = MultiStack(stack_size=stack_size, num_of_stacks=num_stacks)

    for stack_num in range(num_stacks):
        assert stack.is_empty(stack_num)
        assert not stack.is_full(stack_num)
        with pytest.raises(StackEmptyError):
            stack.pop(stack_num)

        for i in range(stack_size-1):
            stack.push(i, stack_num=stack_num)
            assert stack.peek(stack_num) == i
            assert not stack.is_empty(stack_num)
            assert not stack.is_full(stack_num)

        stack.push(999, stack_num=stack_num)
        with pytest.raises(StackFullError):
            stack.push(777, stack_num=stack_num)

        assert not stack.is_empty(stack_num)
        assert stack.is_full(stack_num)

        assert stack.peek(stack_num) == 999
        assert stack.pop(stack_num) == 999
        assert not stack.is_empty(stack_num)
        assert not stack.is_full(stack_num)

        for i in range(stack_size - 2, 0, -1):
            assert stack.peek(stack_num) == i
            assert stack.pop(stack_num) == i
            assert not stack.is_empty(stack_num)
            assert not stack.is_full(stack_num)

        assert stack.peek(stack_num) == 0
        assert stack.pop(stack_num) == 0
        assert stack.is_empty(stack_num)
        assert not stack.is_full(stack_num)
...
def test_stack_does_not_exist():
    s = MultiStack(stack_size=3, num_of_stacks=1)
